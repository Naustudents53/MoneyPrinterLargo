...
def get_movies_cache_path() -> str:
    """
    Gets the path to the Movie Summary cache file.

    Returns:
        path (str): The path to the movies cache file
    """
    return os.path.join(get_cache_path(), 'movies.json')
...
def get_provider_cache_path(provider: str) -> str:
    ...
    Args:
        provider (str): The provider name ("twitter", "youtube", or "movies")
    ...
    if provider == "movies":
        return get_movies_cache_path()

    raise ValueError(f"Unsupported provider '{provider}'. Expected 'twitter', 'youtube', or 'movies'.")
